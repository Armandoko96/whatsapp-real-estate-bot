...
import { uploadImageToDrive, uploadDocToDrive } from '../drive/uploadUtils';
...
const userSessions: Record<string, {
  step: 'awaitingRoute' | 'awaitingInput' | 'awaitingImages' | 'awaitingDesc';
  routeText?: string;
  folderId?: string;
  propertyId?: string;
    ...
    this.client.on('message', async (msg) => {
      ...
      if (session.step === 'awaitingRoute') {
        if (!msg.hasMedia) {
          ...
          const propertyId = generateId({ agent, type, operation, sector, colony }, consecutive);
        ...
        if (msg.hasMedia) {
          ...
          try {
            ...
            for (const img of session.images) {
              ...
              await uploadImageToDrive(drive, session.folderId!, img.fileName, img.buffer, img.mimeType);
            }
            // Crear y subir documento
            const doc = new Document({
              sections: [{ children: [new Paragraph(improved)] }]
            });
            const docBuffer = await Packer.toBuffer(doc);
            console.log('Subiendo documento detalles.docx a Drive...');
            await uploadDocToDrive(drive, session.folderId!, `detalles.docx`, docBuffer);
